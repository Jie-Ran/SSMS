...
class StartPage:
	def __init__(self, parent_window):
		...
		self.window.iconbitmap('genji.ico') # 图标
		self.window.geometry('300x470') # 这里的乘是小x

 
		label = Label(self.window, text="学生成绩管理系统", font=("华文彩云", 25))
		...
		Button(self.window, text="学生登陆", font=tkFont.Font(size=16), command=lambda: StudentPage(self.window), width=30,
			   height=2,fg='black', bg='thistle', activebackground='black', activeforeground='white').pack()
		Button(self.window, text="关于", font=tkFont.Font(size=16), command=lambda: AboutPage(self.window), width=30, height=2,
			   fg='black', bg='lavender', activebackground='black', activeforeground='white').pack()
		Button(self.window, text='退出系统', height=2, font=tkFont.Font(size=16), width=30, command=self.window.destroy,
			   fg='black', bg='lightsalmon', activebackground='black', activeforeground='white').pack()
...
class AdminPage:
	def __init__(self, parent_window):
		...
		label = tk.Label(self.window, text='管理员登陆', bg='lightskyblue', font=('Verdana', 20), width=30, height=2)
		label.pack()
...
class StudentPage:
	def __init__(self, parent_window):
		...
		label = tk.Label(self.window, text='学生登陆', bg='thistle', font=('Verdana', 20), width=30, height=2)
		label.pack()
...
class StudentView:
	def __init__(self, parent_window, student_id):
		...
		label = tk.Label(self.window, text='学生信息查看', bg='lightsalmon', font=('Verdana', 20), width=30, height=2)
		label.pack(pady=20)
...
class AboutPage:
	def __init__(self, parent_window):
		...
		label = tk.Label(self.window, text='学生信息管理系统', bg='lavender', font=('Verdana', 20), width=30, height=2)
		label.pack()
 
		Label(self.window, text='作者：孑然', font=('Verdana', 16)).pack(pady=30)
		Label(self.window, text='个人博客：', font=('Verdana', 14)).pack(pady=5)
		Label(self.window, text='https://www.cnblogs.com/jkxsz2333/', font=('Verdana', 10)).pack(pady=5)
 
		Button(self.window, text="返回首页", width=10, font=tkFont.Font(size=12), command=self.back).pack(pady=100)
